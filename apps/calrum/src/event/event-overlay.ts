...
export class CalrumRootComponent extends connect(store)(LitElement) {
  ...
  stateChanged(state: RootState) {
    if (!isNullOrUndefined(this.opened)) {
      const { change, origin, originalDate } = getOverlayStateSelector(
        state.event
      );
      this.opened = change;
      this.origin = origin;
      ...
      customElements.whenDefined('paper-dialog').then(()=>{
        this.dialog = this.shadowRoot.getElementById('dialog') as any;
        this.opened ? this.dialog.open() : this.dialog.close();
        this.attachCloseEventListener();
      })
    }
  }
  private attachCloseEventListener() {
    ...
    return html`
      <paper-dialog id="dialog">
        ...
        <paper-dialog-scrollable>
          <calrum-event-form
            class="padded-overlay-form"
            id="${this.id}"
            label="${this.label}"
            date="${transformDateToShortString(this.origin)}"
