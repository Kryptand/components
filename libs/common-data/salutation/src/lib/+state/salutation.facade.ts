...
export class SalutationFacade extends SalutationFacadeBase {
    constructor(store: Store<IEntityState<Salutation>>,private http:HttpClient) {
        super(Salutation, store);
    }

    loadPage({page=1,size=25}:IPage, criteria?: any) : Observable<IEntityWithPageInfo<Salutation>>
    {
      console.debug(page);
      console.debug(size);

      const url:string=criteria?`${API_URL}/salutation?page=${page}&limit=${size}&${criteria}`:`${API_URL}/salutation?page=${page}&limit=${size}`;
